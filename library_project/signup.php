...
<body>
		...
		<div class=navbar>
		<a href="Page1.php"><style="position:relative; left:7px; top:7px; align:center; width:40px; height:40px;></a>
		...
		<?php if(is_null($_SESSION['mail'])) echo "<input style='padding:10px; float:right; height:45px;' type=submit class=button name=signIn value='Sign In'>"; ?>                 
			...
			<br><br><br>
			<div class="col-md-2">
				<a href=" page1.php " class=" button"> <button type="button" onclick="alert('want to page1??? ')">signOut</button> </a>   
			</div>
		</div>
...
if(mysqli_query($connect, $sql))
{
	...
	//$connect=mysqli_connect($host,$username,$password,$db_name);
	//$user1="CREATE TABLE student_books (serial VARCHAR(10) UNIQUE,title VARCHAR (50),author VARCHAR(30),mail VARCHAR(30))";//-------CREATING USER SPECIFIC TABLE
...
and register to access world of books...
</p><br><br><br><br>
...
<input class=box type=text name="loadd" placeholder="Local Address" required></input>

<br><br>

<input class=box type=text name="dept" placeholder="Department" required></input>
